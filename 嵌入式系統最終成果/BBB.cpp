...
#define Range_high 8
#define Range_low 0
#define SWITCH "s"//swutch p1 p2 action
#define UP 1
#define DOWN 5
#define SHOOT 9
#define UNSHOOT 0
...
//call get=get(deviceADDR, ACCEL_X_OUT_H)
int get(const char* reg1, const char* reg2) {
	...
	str2 = exec(str);
	return stoi(str2, nullptr, 16);
}
void enablempu6050() {
	int file, I2CBus = 1;//i2c -1
	char namebuf[64];
	snprintf(namebuf, sizeof(namebuf), "/dev/i2c-%d", I2CBus);//namebuf=/dev/i2c-1
	if ((file = open(namebuf, O_RDWR)) < 0) {//initial i2c-1 ability
		cout << "error0" << endl;
	}
	if (ioctl(file, I2C_SLAVE, 0x68) < 0) {//control 0x68 position
		cout << "error1" << endl;
	}
	int result = i2c_smbus_write_byte_data(file, 0x6B, 0x00); //writebyte 0x00 to register0x6b
	if (result == -1) {
		cout << "error2" << endl;
	}
...
int main() {
	...
	/*while (true) {
		char mDirection[1];
		int mpu_ax = get(deviceADDR, ACCEL_Y_OUT_H);
		int mpu_ay = get(deviceADDR, ACCEL_Z_OUT_H);
		int mpu_az = get(deviceADDR, ACCEL_X_OUT_H);
		if (mpu_ay > 150) {mpu_ay =  mpu_ay-256;}
		if (mpu_ax > 150) {mpu_ax = mpu_ax - 256;}
		if (mpu_az > 150) {mpu_az = mpu_az - 256;}
		
		if (mpu_ax < -15) { mpu_ax = -1; }
		else if(mpu_ax>15){ mpu_ax = 1; }
		else { mpu_ax = 0; }
		if (mpu_ay < -15) { mpu_ay = -1; }
		else if (mpu_ay>15) { mpu_ay = 1; }
		else { mpu_ay = 0; }
		if (mpu_ax*mpu_ay == 1) {
			if(mpu_ax ==1){ sprintf(mDirection, "%d", DR); }
			else{ sprintf(mDirection, "%d", UL); }
		}
		else if (mpu_ax*mpu_ay == -1) {
			if (mpu_ax == 1) { sprintf(mDirection, "%d", UR); }
			else { sprintf(mDirection, "%d", DL); }
		}
		else if (mpu_ax*mpu_ay == 0) {
			sprintf(mDirection, "%d", STOP);
			if (mpu_ax == 1) { sprintf(mDirection, "%d", RIGHT); }
			else if(mpu_ax ==-1){ sprintf(mDirection, "%d", LEFT); }
			if (mpu_ay == 1) { sprintf(mDirection, "%d", DOWN); }
			else if (mpu_ay == -1) { sprintf(mDirection, "%d", UP); }
		}
		stream = fopen(bluetooth, "r+");
		fwrite(mDirection, 1, 1, stream);
		fclose(stream);
	...
	while (1) {
		...
		float xa, ya, za;
		int AxisX = 0, AxisY = 0;
		xa = dof.calcAccel(dof.ax);

		if (xa < -0.3) {  AxisX = 1; }
		else if (xa > 0.2) { AxisX = -1; }
		ya = dof.calcAccel(dof.ay);
		if (ya > 0.15) {AxisY = 1; }
		else if (ya < -0.35) { AxisY = -1; }
		if(AxisX*AxisY==1){//1*1 or -1*-1
			if (AxisX == -1) {
				sprintf(Direction, "%d", DL);
		}
			else { sprintf(Direction, "%d", UR); }
		}
		else if (AxisX*AxisY == -1) {
			if (AxisX == -1) {
				sprintf(Direction, "%d", UL);
			}
			else { sprintf(Direction, "%d", DR); }
		}
		else {
			if (AxisX == 0) 
			{
				if (AxisY == 0) {
					sprintf(Direction, "%d", STOP);
				}
				else if (AxisY == 1) {
					sprintf(Direction, "%d", UP);
				}
				else if (AxisY == -1) {
					sprintf(Direction, "%d", DOWN);
				}
			}
			else {
				if (AxisX == 1) {
					sprintf(Direction, "%d", RIGHT);
				}
				else
				{
					sprintf(Direction, "%d", LEFT);
				}
			}
		...
		stream = fopen(bluetooth, "r+");
		fwrite(Direction,1,1, stream);
		fclose(stream);
		stream = fopen(ADC_DIR"iio:device0/in_voltage0_raw", "r+");
		char button[10];
		fscanf(stream, "%s", button);
		int voltge = atoi(button);
		cout << voltge << endl;
		fclose(stream);
		if (voltge < Range_high&&voltge >= Range_low) {
			sprintf(Direction, "%d", SHOOT);
		}
		else { sprintf(Direction, "%d", UNSHOOT); }
		stream = fopen(bluetooth, "r+");
		fwrite(Direction, 1, 1, stream);
		fclose(stream);
		////////////////////////////////
...
void LSM9DS0::initGyro()
{
	...
	// Temporary !!! For testing !!! Remove !!! Or make useful !!! dinci:??
...
{
	if (ioctl(file, I2C_SLAVE, address) < 0) {//address=LSM9DS0_XM 0x1d file not data but status of open()
		cout << "I2C_SLAVE address " << address << " failed..." << endl;
	}
	int result = i2c_smbus_write_byte_data(file, subAddress, data);//write (unsigned char)data to subaddress example of subaddress 0x12 
	if (result == -1) {												//›”file”‡ŒÂŽåÚŒû(0x1d)“I‘æ(subaddress)ŒÂŽb‘¶Ší›“üdata Œ©Žè‹@Æ•Ð
